...
release candidates, and CI now runs it there on every change; the bound
stays `polars>=1.44.1,<2` until 2.0 itself is released and the same suite
is green against it. Along the way, testing against Polars 2 turned up two
bugs that Polars 1 users hit today, both fixed here. No seeded output
changes, and no spec file needs migrating.

### Fixed

- **`choices` on a `Decimal`, or a `Datetime` not in microseconds.** Drift
  failed on a `Decimal` column with `choices`; validation raised an error on
  a `Datetime("ms")` column with them, and on a `Datetime("ns")` column
  rejected the column's own generated data. The choices reached Polars'
  `is_in` as a Python list, which arrives at full precision or in
  microseconds. Validation, a rule's choices and drift now compare
  against the choices as a Series of the column's own dtype -- which is also
  what Polars 2 requires of an integer choice on a float column.

### Added

- **`scan()` names itself in `explain()` on Polars 2.** A scan's node reads
  `PYTHON[polspec: Orders] SCAN`, with an `INFO:` line giving the rows, the
  seed and the batches. Whether to pass the labels is read off
  `register_io_source`'s signature, so on Polars 1 the call is unchanged
  and the header stays `PYTHON SCAN`.

- **Polars 2's `Map` dtype has a spec-file form**, `{Map: {key: <dtype>,
  value: <dtype>}}`, so a profiled map column can be saved -- `polspec
  schema infer` on a file with one no longer fails. A `Map` column
  declares, validates, profiles and drifts by its dtype; `generate()`
  refuses it by name until drawing its values arrives.

### Internal

- A `polars-next` CI job runs the suite on the newest Polars 2
  pre-release, informational until 2.0 is final.

## [0.10.0] - 2026-09-25

**One seeded change, once: every `unique=True` column generates different
values for the same seed** -- in `generate()` too, not only when batched --
because a unique column is now a permutation of its value space. That is
what makes it unique across a whole frame however it is produced: until
now, `generate_batches`, `scan()` and the sinks drew a unique column afresh
in every batch and the same way each time, so a key repeated every batch
(0.9.2 warned; this release fixes it). Nothing else's seeded output
changes, and no spec file needs migrating.

Around it: `Orders.read(path)` reads a file in the spec's terms,
`report.passing_rows()` and `polspec validate --output / --failing` split
a file into what passed and what did not, a list's elements can be null,
and the test suite is organised by what each file covers.

### Changed

- **A unique column is a permutation, so it is unique across the whole
  frame.** Its value at row `r` is the `π(r)`-th value of its space, where
  `π` is a keyed permutation -- a Feistel network walking its cycle back
  into range -- so distinct rows get distinct values and a row's value
  depends only on the row and the seed. Every unique kind: integers,
  booleans, choices and `Enum` (an offset into the domain), floats (an
  evenly spaced grid over the bounds, four ulps apart at the widest
  magnitude so a cast cannot merge two points), dates, `Decimal`,
  `Int128`, `Float16`, and strings and formats, whose values decode from an
  index one digit per template part. A batch of `generate_batches`, a
  `scan()` or a sink is now the whole frame's slice for a unique column as
  for any other, and the 0.9.2 warning is gone with what it warned about.
  `__unique_together__` and a foreign key drawn without replacement are
  still distinct only within a batch.

  Two consequences to know. A unique string is drawn uniformly from every
  string its lengths allow, so it favours its longest length -- 98% of a
  `string_length=(5, 15)` column is fifteen characters. And a null row
  still takes its place in the permutation, so a nullable unique column's
  domain has to hold every row, nulls included.

- **Much faster, and bounded in memory.** The unique draw no longer holds a
  set of the values it has drawn, and fills chunk by chunk in parallel
  like any column: 10 million rows from a crowded integer domain take
  0.016s where they took 1.42s, and a million unique strings 0.014s where
  they took 0.185s.

### Added

- **`Orders.read(path)`** (and `polspec.read(spec, path)`): a data file in
  the spec's terms. The extension picks the reader -- the set `polspec
  validate` reads -- Polars' options pass through (`separator=";"`), and
  each column the spec declares as a `Date`, `Datetime` or `Time` that
  arrived as text is parsed when every value parses, leaving date-shaped
  text in a `String` column alone. Nothing else is cast: the frame goes to
  `inspect()` or `validate(cast=True)`. "Checking a file you were given"
  starts with it, and the CLI reads through the same code.

- **`report.passing_rows()`**: every row no finding touched, lazily and in
  order -- the complement of `failing_rows()`, so the two split a frame
  into what to keep and what to send back. A structural finding judges the
  whole frame, so it refuses until that is fixed.

- **`polspec validate --output PATH --failing PATH`**: the passing rows,
  validated again with `cast=True` so the file is one the spec accepts and
  typed as it declares, and the failing rows with the claim each broke.
  The exit status is unchanged, so a pipeline can quarantine the bad rows
  and still fail the step; notes go to stderr so `--json` stays one
  document.

- **`element_null_probability`**: how often an element inside a list is
  null. 0 by default -- elements are never null, as before -- and any rate
  up to 1 accepts them, on a `List` or an `Array`, a struct or list element
  null as a whole. Generation, validation, the spec file (an added optional
  key), `diff` and `drift` (which now measures element nulls), the
  profiler, the data dictionary and `estimated_size` all read it.

### Internal

- **The test suite in folders, by what each file covers**: `contracts/`
  (the round trip, and every "each field has an entry" test in one
  `test_parity.py`), `declaration/`, `generation/`, `validation/`,
  `drift/`, `serialization/`, `features/` (one file per feature across
  every verb), `cli/` and `docs/`. The same 1135 tests collected before and
  after the move; a test now fails on a test file at the top level or
  without a docstring.

## [0.9.2] - 2026-09-24

The fixes from a review of 0.9.1, and the three numeric dtypes 0.9.0's
"every dtype generates" missed: `Int128`, `UInt128` and `Float16` now
generate, and a census test fails the day Polars adds one that does not.
The one to know about: a `unique=True` column repeats its values in every
batch of `generate_batches`, `scan()` and the sinks -- that has been true
since batching arrived, and the docs understated it. This release warns
when it happens and says so plainly; 0.10.0 makes the draw batch-stable.
No seeded output of an existing spec changes, and no spec file needs
migrating.

### Added

- **`Int128`, `UInt128` and `Float16` generate.** They declared and
  validated before, but `generate()` failed with an engine error and
  `to_yaml` could not write them. Each is drawn through a type the engine
  has and cast back, as a `Decimal` is: a 128-bit integer through 64 bits
  -- bounds past that are refused at `generate()`, naming the column,
  while validation checks the full range -- and a half as a single,
  rounded to the nearest half. The draw stays between the halves nearest
  each bound from inside, so rounding never carries a value past one;
  bounds too close together to hold any half are refused where they are
  written, and a unique half is drawn from the finite set of halves
  between its bounds. YAML, `to_python`, `estimated_size` and
  `from_dataframe` know all three.

  "Every dtype" now reads *every dtype that holds data*: `Null`, `Object`,
  `Unknown` and extension types declare and validate by dtype, and
  `generate()` refuses each by name. A census test walks every dtype class
  Polars exports and fails on one that is neither.

### Changed

- **A unique column that repeats across batches says so.** Under
  `generate_batches`, `scan()` and every `sink_*`, a `unique=True`
  column's draw starts over in each batch the same way, so every batch
  holds the same values -- the docs called it "drawn per batch", which
  read as independent draws. polspec now warns once, as the second batch
  is drawn, naming the columns and pointing at `generate()` for a frame
  that must be unique throughout; a run that fits in one batch, or a scan
  that never collects the column, stays silent. The limitations and
  generating pages describe what happens.

### Fixed

- **A finding on a zoned `Datetime` no longer panics on Windows.** A
  finding's samples are Python values, and a zoned datetime needs the IANA
  database to become one; Windows ships none, so Polars panicked -- past
  any `except Exception` -- on the first finding on such a column. `tzdata`
  is now a dependency on Windows.

- **`ColSpec(pl.Struct({}))` generates a `Struct({})` column**, where it
  generated an untyped null one.

- **Docs**: `comparison.md` no longer says polspec has no drift detection,
  and `architecture.md`'s module table gains the seven modules it had
  fallen behind on, pinned by a test.

### Internal

- Warnings are errors in the test suite, so a warning is asserted where it
  is expected -- with `pytest.warns`, or the doc runner's new
  `<!-- docs: warns -->` marker -- and a failure everywhere else.
