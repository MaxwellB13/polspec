...
/// kind cannot be added to two of the three and forgotten in the last.
macro_rules! declare_kinds {
    ($($variant:ident => $name:literal),+ $(,)?) => {
        /// The physical kind of value a column is filled with.
        ///
        /// Python maps every Polars dtype onto one of these: temporal dtypes
        /// cross as the integer they store, and any column with a finite
        /// domain (`choices`, an `Enum`, a capacity-limited `Categorical`)
        /// crosses as `Index`, receiving indices into that domain back.
...
/// Everything `ColumnPlan::build` reads, as one struct.
///
/// A struct rather than a dozen positional arguments: every field is named at
/// the call site, a caller states only what it means to change and leaves the
/// rest to `..Default::default()`, and a new field is one line here instead of
/// an edit to every constructor and test in the crate.
#[derive(Default)]
...
///
/// The fields Python reads back are exposed with `#[pyo3(get)]`; `kind` and
/// `distribution` are the two exceptions, being Rust enums Python has no
/// binding for, so each has a `#[getter]` handing back its canonical name.
...
impl ColumnPlan {
    ...
    /// The kind's canonical name, rather than an enum Python has no binding for.
    #[getter]
    fn kind(&self) -> &'static str {
        self.kind.name()
    }

    /// The distribution's canonical name, for the same reason as `kind`.
    #[getter]
...
mod tests {
    ...
    /// A plan of one kind, with every other field left at its default.
    ...
    fn index_needs_a_domain_and_matching_weights() {
        ...
        let bad = ColumnPlan::build(PlanArgs {
            name: "c".into(),
            kind: "index",
            n_categories: Some(3),
    ...
    fn bool_probability_comes_from_weights_then_params() {
        let from_weights = ColumnPlan::build(PlanArgs {
            name: "b".into(),
            kind: "bool",
        ...
        let params = HashMap::from([("p".to_string(), 0.2)]);
        let from_params = ColumnPlan::build(PlanArgs {
            name: "b".into(),
            kind: "bool",
            params: Some(&params),
            ..Default::default()
