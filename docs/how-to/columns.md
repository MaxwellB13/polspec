...
That is every dtype that holds data, and Polars 2's `Map(key, value)` too:
a `Map` is the list of `{key, value}` entries it is, so `list_length` and
`fields` describe it as they would a `List(Struct)`, and every map
generates with its keys present and distinct. `Null`, `Object`, `Unknown`
and extension types declare and validate by dtype, but hold nothing
polspec can draw, so `generate()` refuses each by name. A 128-bit integer is drawn
